...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
				(event.url ?
					'data-toggle="modal" href="' + htmlEscape(event.url) + '"' :
					''
					) +
				(skinCss ?
					' style="' + skinCss + '"' :
			...
			'>' +
				...
				'<i class="icon-check" style="line-height:18px;"></i></div>' +
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content"><i class="icon-check" style="line-height:18px;"></i>' +
					(timeText ?
						'<div class="fc-time"' +
